...
//   Current version: V1_R22 (see Revision History block at end of file for the
//   full chronological log; the newest entries are summarized below).
//
// V1_R22 (this revision): adds endpoint-compatible continuous source exclusion,
...
const string AMBIENT_RNA_VERSION = "3.2-ck-phase1-mechanism-resolution";
const string AMBIENT_RNA_VERSION_MSG = "Adds endpoint-compatible continuous source exclusion, explicit mass accounting, parent-axis and conditional-information diagnostics, and selector-limited r-C likelihood surfaces while preserving existing behavior when the new options are unused";
...
bool contamFinder3::reclassify_cells(){
    ...
    // Low-data unit removal gate. On degenerate (low-cell) units, every cell can
    // hit the unclassifiable branches below ((A) no identity with a positive LLR
    // margin, (B) populate_llr_table fails for lack of usable SNP data), which
    // would delete the entire active set, empty assn, and feed a zero-row
    // component matrix to the next iteration's mixture solve. Below the floor,
    // suppress all removal so unclassifiable cells are frozen in place. At or
    // above the floor, removal behavior is unchanged (identical to the deployed
    // binary). assn.size() here is the active-cell count at pass entry, counting
    // every barcode including tetraploid-locked ones; assn is not modified
    // between function entry and the reclassification loop.
    const size_t MIN_CELLS_FOR_REMOVAL = 100;
    const bool suppress_removal = (assn.size() < MIN_CELLS_FOR_REMOVAL);
    if (suppress_removal){
        fprintf(stderr, "  Low-data unit (%zu cells < %zu): suppressing cell "
            "removal in reclassification\n", assn.size(), MIN_CELLS_FOR_REMOVAL);
    }
...
//                        c_cell_map >= contam_rate[a->first] &&  
                ...
                // Frozen in place on low-data units instead of deleted.
                if (!suppress_removal){
            ...
            // Branch (B): populate_llr_table failed (insufficient usable SNP
            // data for >=2 candidate identities). Frozen in place on low-data
            // units instead of deleted.
            if (!suppress_removal){
                cell_rm.insert(a->first);
            }
        }
