// ============================================================================
// quant3_contam_ap.cpp
// Three-component contamination model driver with r-feedback and adaptive prior
//
...
// a new .allele_ratio file with per-cell genome A fraction estimates.
//
// New flags:
//   --r_feedback     Feed per-cell r estimates back into ambient profile
//   --adaptive_prior Auto-detect pathological distributions and apply fixed prior
//   --adaptive_mean, --adaptive_init_var, --adaptive_thresh  Tuning parameters
// ============================================================================
 ...
 */
void help(int code){
    fprintf(stderr, "quant3_contam [OPTIONS]\n");
    ...
    fprintf(stderr, "    --fi_weight  Use continuous Fisher Information weighting instead of the\n");
    fprintf(stderr, "        hard adaptive filter for SNP category selection. Categories are weighted\n");
    fprintf(stderr, "        by (p_c - p_e)^2, giving low-signal categories proportionally less\n");
    fprintf(stderr, "        influence rather than dropping them entirely. Default off.\n");
    fprintf(stderr, "    --leave_one_out  Compute per-identity leave-one-out ambient profiles.\n");
    fprintf(stderr, "        For each singlet identity, the ambient profile used to estimate its\n");
    fprintf(stderr, "        contamination rate excludes that identity's contribution, reducing\n");
    fprintf(stderr, "        circular bias. Default off.\n");
    fprintf(stderr, "    --prior_mean FLOAT  Override the data-driven (empirical Bayes) prior mean\n");
    fprintf(stderr, "        for per-cell contamination rate with a fixed value. Default = -1.0\n");
    fprintf(stderr, "        (disabled; use data-driven prior).\n");
    fprintf(stderr, "    --prior_var FLOAT  Override the data-driven prior variance with a fixed\n");
    fprintf(stderr, "        value. Default = -1.0 (disabled; use data-driven prior).\n");
    fprintf(stderr, "===== AMBIENT PROFILE REFINEMENTS =====\n");
    fprintf(stderr, "    --r_feedback  Enable per-cell allele ratio feedback into ambient\n");
    fprintf(stderr, "        profile estimation. Heterotypic cells use their estimated r\n");
    fprintf(stderr, "        instead of 0.5 when contributing to the ambient mixture.\n");
    fprintf(stderr, "    --adaptive_prior  Enable adaptive prior: start with empirical Bayes,\n");
    fprintf(stderr, "        switch to fixed prior with iterative variance shrinking if the\n");
    fprintf(stderr, "        contamination distribution looks pathological.\n");
    fprintf(stderr, "    --adaptive_mean FLOAT  The mean for the adaptive fixed prior\n");
    fprintf(stderr, "        (default 0.05).\n");
    fprintf(stderr, "    --adaptive_init_var FLOAT  Starting variance for adaptive prior\n");
    fprintf(stderr, "        (default 0.04).\n");
...
void infer_from_genotypes(string& output_prefix,
    ...
    double prior_mean,
    double prior_var,
    // Ambient profile refinement parameters
    bool r_feedback,
    bool adaptive_prior,
...
all possible individuals\n", idfile_doublet.c_str());
    ...
    while (delta > delta_thresh){
        ...
        // Combo: FI-weight, LOO, and fixed prior
        if (fi_weight){
            cf.set_fi_weight(true);
        }
        if (leave_one_out){
            cf.set_use_loo(true);
            if (nits == 0){
        ...
        if (prior_mean > 0 && prior_var > 0){
            cf.set_user_prior(prior_mean, prior_var);
        }
...
on mixture proportions...\n");
    ...
    // Only heterotypic tetraploid cells have allele_ratio estimates
    {
...
int main(int argc, char *argv[]) {    
    ...
    static struct option long_options[] = {
       ...
       {"fi_weight", no_argument, 0, 1002},
       {"leave_one_out", no_argument, 0, 1003},
    ...
    // Combo flags
    bool fi_weight = false;
    bool leave_one_out = false;
    double prior_mean = -1.0;
    double prior_var = -1.0;

    // Ambient profile refinement flags
    ...
    while((ch = getopt_long(argc, argv, "o:e:g:G:E:l:N:i:I:n:b:D:B:F:M:t:c:T:X:ARrsCSUdwh", long_options, &option_index )) != -1){
        switch(ch){
            ...
            case 1003:
                ...
                break;
            case 1004:
                prior_mean = atof(optarg);
                break;
            case 1005:
                prior_var = atof(optarg);
                break;
            case 1006:
                r_feedback = true;
                break;
            ...
            case 1007:
                ...
                break;
            case 1008:
                adaptive_init_var = atof(optarg);
                break;
            case 1009:
                adaptive_thresh = atof(optarg);
                break;
            case 'g':
