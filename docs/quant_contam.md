...
* `[output_prefix].contam_rate` lists per-cell contamination estimates. Each line is three tab-separated columns: cell barcode, contamination rate (a decimal between 0 and 1 indicating the fraction of that cell's RNA attributed to ambient contamination), and the standard error of that estimate. The rate is a maximum *a posteriori* estimate (Empirical Bayes shrinkage toward the dataset mean), not a maximum likelihood estimate. The SE is derived from Fisher information and will be 0 in two situations: the optimizer did not find a root within the valid range (0, 1) and fell back to a boundary value (rates of exactly 0 or 1 will always have SE = 0 because Fisher information is undefined at the boundaries), or the optimizer found the MLE but the SE calculation did not converge. A nonzero SE on an interior rate estimate is reliable; SE = 0 on an interior estimate indicates a numerical issue for that cell and should be interpreted with caution.
* `[output_prefix].decontam.assignments` is the refined file of cell-individual assignments, in the same format as the `.assignments` file produced by `demux_vcf`. Assignments are re-inferred after modeling the ambient RNA profile, so identities here may differ from those in the original `.assignments` file for highly contaminated cells.
